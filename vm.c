...
// return 1 if virtual address is found in p->queue ,return -1 otherwise.
int searchqueue(char *va) {
  struct proc *p = myproc();
  // Find a matching element
  for (int i = 0; i < CLOCKSIZE; i++) {
    if ((p->clockqueue[i] != (char *)-1) && (p->clockqueue[i] == va)) {
      return 1;
    }
  }
  return -1;
...
int getpgtable(struct pt_entry* pt_entries, int num, int wsetOnly) {
  ...
  for (;; uva -= PGSIZE) {
    ...
    if (wsetOnly) {
      // Page exists in working set
      if ((searchqueue((char*)uva) != -1)) {
...
int dump_rawphymem(uint physical_addr, char * buffer) {
  cprintf("before buffer: %x\n", buffer);
  *buffer = *buffer;
  cprintf("after buffer: %x\n", buffer);
...
int enqueue(char *va, struct proc* p) {
  cprintf("enqueue start!\n");
  for(;;) {

    cprintf("before: p->qhead: %x\n", p->qhead);
    p->qhead = (p->qhead + 1) % CLOCKSIZE;
    char* uva = (char*)PGROUNDDOWN((uint)p->clockqueue[p->qhead]);
    cprintf("p->qhead: %x, uva: %x\n", p->qhead, uva);

    pte_t* pte = walkpgdir(p->pgdir, uva, 0);

    // empty space exists in queue
    if (p->clockqueue[p->qhead] == (char *)-1) {
      p->clockqueue[p->qhead] = va;
      cprintf("in if, no existing va!!: p->qhead: %x, uva: %x\n", p->qhead, uva);
      return 0;
    } else if (!((*pte) & PTE_A)) { // pte is not accessed
      // encrypt evicted page
      cprintf("in if, va ari!!: p->qhead: %x, uva: %x, p->clockqueue[p->qhead]:%x\n", 
                    p->qhead, uva, p->clockqueue[p->qhead]);
      if (mencrypt(p->clockqueue[p->qhead], 1) != 0) {
        return -1;
      }
      // evict and new page
      p->clockqueue[p->qhead] = va;
      cprintf("after: p->qhead: %x, uva: %x\n", p->qhead, uva);
      return 0;
    } else { // pte is accessed so change PTE_A == 0
      *pte = *pte & ~PTE_A;
    }
  } // end of while
} // end of function
...
// dequeue page
// success: return 0,fail: return -1;
int dequeue(char *virtual_address) {
  struct proc *curproc = myproc();
  int tail = curproc->qhead;
  int qptr = -1;

  // Search for the location to delete
  for (int i = 0; i < CLOCKSIZE; i++) {
    if (curproc->clockqueue[(curproc->qhead+i)%CLOCKSIZE] == virtual_address) {
      qptr = (curproc->qhead+i)%CLOCKSIZE;
    }
  }
  ...
  if (qptr != -1) {
    ...
    for (int i = qptr; i != tail; i = (i + 1) % CLOCKSIZE) {
      curproc->clockqueue[i] = curproc->clockqueue[(i + 1) % CLOCKSIZE];
    }

    // 削除した分をずらす
    curproc->clockqueue[tail] = (char *)-1;
    curproc->qhead = curproc->qhead == 0 ? CLOCKSIZE - 1 : curproc->qhead - 1;
    return 0;
  }
  return -1;
}
